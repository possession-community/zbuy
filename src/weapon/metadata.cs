...
namespace Zbuy
{
    ...
    public static class WeaponMetadata
    {
        private static readonly Dictionary<string, int> DefaultPrices = new(StringComparer.OrdinalIgnoreCase);

        static WeaponMetadata()
        {
            var weaponMetas = Load();
            foreach (var meta in weaponMetas)
            {
                DefaultPrices[meta.WeaponName] = meta.Price;
            }
        }

        public static int GetDefaultPrice(string weaponName)
        {
            return DefaultPrices.TryGetValue(weaponName, out int price) ? price : 0;
        }

        public static List<WeaponMeta> Load()
        {
            return new List<WeaponMeta>
            {
                new()
                {
